...
export function getSeededRecyclers(
  ...
  return TEMPLATES.map((t, i) => {
    ...
    return {
      ...
      address: origin
        ...
        : "Demo location — enable location for real distances",
...
export function findRecyclersForWasteType(
  ...
  return recyclers.filter((r) => r.acceptedWaste.includes(wasteType));
}
...
export function getBestRecycler(
  ...
  return getNearestRecyclers(recyclers, wasteType, userLocation)[0] ?? null;
}
...
export function recyclerSummary(
  ...
  const distance = userLocation
    ? formatDistanceKm(haversineDistanceKm(userLocation, recycler))
    : "Unknown distance";
