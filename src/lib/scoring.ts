...
function labelBonus(product: Product): number {
  ...
  if (labels.includes("fair trade") || labels.includes("fairtrade")) bonus += 4;

  if (labels.includes("rainforest alliance") || labels.includes("b corp"))
    bonus += 3;
...
export function computeScore(
  ...
  const total = Math.max(
    0,
    Math.min(100, Math.round(base + lBonus - pPenalty + rBonus)),
  );
...
export function explainScore(score: ScoreBreakdown, product: Product): string {
...
export function getScoreInsights(score: ScoreBreakdown, product: Product) {
  const strengths: string[] = [];

  const weaknesses: string[] = [];

  if (score.base >= 70) strengths.push("Low-impact product category.");

  if (score.labelBonus > 0)
    strengths.push("Recognized sustainability certifications.");

  if (score.recyclerBonus > 0) strengths.push("Nearby recycler available.");

  if (score.packagingPenalty >= 8)
    weaknesses.push("Packaging is difficult to recycle.");

  if (score.base < 50) weaknesses.push("High-emission product category.");

  if (score.total < 50)
    weaknesses.push("Overall sustainability score is below average.");
...
export function estimateKgCo2(score: ScoreBreakdown): number {
  const cleanliness = score.total / 100;

  const base = 2.4;

  return Math.round(base * (1.6 - cleanliness) * 100) / 100;
}
