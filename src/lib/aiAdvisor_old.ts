...
function adviseOnWaste(ctx: AdvisorContext): string {
  ...
  const distance = ctx.userLocation
...
function predictPickupAndValue(ctx: AdvisorContext): string {
  ...
  const valuePerKg: Record<WasteTypeId, number> = {
    metal: 0.85,
    glass: 0.05,
    paper: 0.08,
    plastic: 0.12,
...
async function askGemini(
  ...
  if (!apiKey) {
    console.warn(
      "[aiAdvisor] No EXPO_PUBLIC_GEMINI_API_KEY set — using local advisor.",
    );
    return null;
  }
  ...
  try {
      ...
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`,
      {
        ...
        body: JSON.stringify({
          contents: [
            {
              role: "user",
              parts: [{ text: buildGeminiInput(promptId, ctx) }],
            },
          ],
    ...
    if (!response.ok) {
      ...
      console.warn(
        `[aiAdvisor] Gemini API error ${response.status}: ${errText}`,
      );
      return null;
    }
...
export async function askAdvisor(
  ...
  const cleanAnswer = geminiAnswer
    ? geminiAnswer.replace(/[\u200B-\u200D\uFEFF]/g, "").trim()
    : "";
